...
- 자동 핸들의 반복 경계 감속과 .blend 액션 보존 누락을 추가 발견해 수정했다. 새 호흡만 주기형 연속 곡선으로 교체한 후 240Hz 원본 곡선·Unity 호흡/연결 검사와 렌더/빌드를 재실행했다. 기존 동작 회귀는 앞선 전체 실행에서 통과했고, 마지막 생성 후 기존 8개 클립/메시/골격 해시 일치를 다시 확인했다. `AdultCommonIdleCurves.json`, `AdultCommonIdleVerification.txt`, `Logs/idle-final.log`가 최종 근거다.
- Windows v0.93: `Logs/AdultRabbitMotionPlayer/TinyDaysAdultRabbitMotion.exe`, 데이터 갱신 **2026-09-24 21:33:43**. `COMMON_IDLE_OK` / `COMMON_IDLE_SEQUENCE_OK` / `ADULT_RABBIT_MOTION_PLAYER_OK` 확인. 프로세스를 8초 시작해 엔진/그래픽/입력 초기화와 예외 없는 로그를 확인하고 이 테스트 프로세스만 종료했다(`Logs/idle-player-smoke.log`). 실제 화면·버튼/마우스 조작 검증은 하지 않았다.
- `Captures/CommonIdle/StandBreathToRunV093.mp4`, `SitBreathToWalkV093.mp4`는 각각 8초·30fps·1× 자동 렌더다. 자세와 전환의 연속 표본을 확인했으며 실제 Game 녹화나 사용자 품질 승인은 아니다. **앉기·서기 기본 호흡과 이동 연결 사용자 확인 대기**. 농가·눕기·추가 변형·GitHub 업로드는 진행하지 않았다.

- **최신 v0.92 (2026-09-24):** 상체 고정 구간·가속도 연결·머리 가변 역회전·손의 곡선과 한 프레임 시차를 수정했다. 1.6초/1.8초·끝 자세·걷기/달리기·단순 상의 흔들림을 유지한다. v0.91 각도/양손 좌표를 새로 측정해 `AdultRabbitSitMotionBeforeV092.csv`로 보존했다. 이전 렌더/영상은 `Captures/AdultRabbitSitBeforeV092`, .blend는 로컬 `Logs/BeforeV092`에 보존했다.
- 초기 손 시차에서 팔 도달 범위 초과를 발견했고 원래 길이를 유지하도록 손 경로를 보완했다. 최종 생성 도달 범위 초과 0, Unity 관절 길이 차이 약 0.001mm. 오른손 지지 종료 +1/30초를 포함한 고정점 이동 최대 0.280mm, 접촉 간격 4.473~4.821mm, 바지 최저 +2.741mm, 신발 최저 +1.019mm다. 배속/프레임률·중간 일시정지·끝 자세·복원 검사 통과.
- 이전 상체 고정 구간의 각도 범위: 앉기 0.010→2.230도, 일어서기 0.029→1.150도. 상체 최대 각가속도 726.056→594.802 / 1670.492→1189.536도/s², 머리 최대 각가속도 369.071→349.863 / 317.745→295.912도/s². 골반 중간 정지 묶음은 0 유지다. 일어서기 손 최고 속도는 약 1.198→1.269/1.301m/s로 소폭 증가했으므로 모든 부위가 느려졌다고 해석하지 않는다. 수치와 근거는 `AdultRabbitSitSmoothnessV092.json`과 검토 문서를 따른다.
- 기존 걷기/달리기·리듬/부드러움 회귀 통과. 최종 손 경로만 추가 조정한 뒤 앉기 검증·영상·Windows 빌드를 재실행했다. 최종 자산의 기존 5개 동작·메시·바인드 골격 해시 일치 확인. `Logs/v092-AdultRabbitSitChecks.Execute.log`, `Logs/v092-AdultRabbitSitChecks.Sequence.log`, `Logs/v092-AdultRabbitMotionBuilder.BuildPlayer.log` 성공 확인.
- Windows v0.92: `Logs/AdultRabbitMotionPlayer/TinyDaysAdultRabbitMotion.exe`, 데이터 갱신 2026-09-24 20:46:38. 1×·30fps·4.2초 자동 영상 `Captures/AdultRabbitSit/SitStandV092.mp4`, `QuarterV092.mp4`, `BeforeLeft_AfterRight_V092.mp4`(왼쪽 v0.91/오른쪽 v0.92). 정면/측면/비스듬한 연속 표본 확인. 실제 프로그램 입력은 미검증이며 **앉기·일어서기 연결감 사용자 확인 대기**. 상의 관통은 미세 보완하지 않았고 GitHub 업로드 없음.

- **최신 v0.91 (2026-09-24):** 앉기/일어서기의 중간 포즈별 정지를 연속 곡선으로 교체하고 상체 숙임·팔/발 정리·후행을 재배치했다. 1.6초/1.8초, 기존 앉은 골격 자세·걷기/달리기·카메라는 유지한다. v0.90에 따라 앉기용 면 추적 코트 보정은 작은 밑단 흔들림으로 대체했고 원본 메시와 기존 이동 옷감 보정은 보존했다.
- 앉기 검사 통과: 손/발 고정점 이동 최대 0.280mm, 접촉 간격 4.473~4.824mm, 신발 최저 +1.019mm, 바지 최저 +2.789mm, 최종 착석 4.657mm. 240Hz 전환, 30/60/120fps×0.5×/1×·반복/반대 요청·일시정지·초기화·이동 후 착석을 직접 호출로 확인했다. 관절 길이 차이 약 0.001mm, 무릎 좌우 간격 최소 296.092mm다.
- 전후 곡선: 골반 중간 감속 묶음 각각 2→0회, 앉기 머리 최대 가속도 46.778→3.585m/s², 착석 후 머리 최대 상하 속도 2.152→0.293m/s. `AdultRabbitSitMotionBeforeV091.csv`와 `AdultRabbitSitMotion.csv`, `AdultRabbitSitSmoothnessV091.json`이 근거다. 기존 5개 클립·메시·바인드 뼈 해시는 이전 자산과 동일하다.
- 초기 시도에서 팔 도달 범위 초과와 준비 중 바지 바닥 침범을 발견해 손 위치·상승 타이밍을 수정했다. 코트의 다리/바닥 교차는 일부 남으며 사용자 합의대로 미세 보완하지 않는다. 기존 걷기/달리기·리듬/부드러움 회귀와 새 앉기 검사 통과. 추가한 전환 중 일시정지/재개(옷 포함) 및 프레임률별 모든 렌더러의 끝 자세 비교도 통과했다(`Logs/sit-v091-final-check.log`).
- Windows v0.91 빌드 완료: `Logs/AdultRabbitMotionPlayer/TinyDaysAdultRabbitMotion.exe`, 데이터 갱신 2026-09-24 19:55:31. `ADULT_RABBIT_MOTION_OK`/`ADULT_RABBIT_RUN_OK`/`RUN_SMOOTHNESS_OK`/`ADULT_SIT_OK`/`ADULT_SIT_SEQUENCE_OK`/`ADULT_RABBIT_MOTION_PLAYER_OK` 확인. 새 Python 곡선 비교는 Blender의 내장 Python으로 재빌드 절차에 연결했다.
- 정면·측면·후측면 주요 자세와 연속 표본 확인. 1×·30fps·4.2초 자동 영상은 `Captures/AdultRabbitSit/SitStandV091.mp4`, 같은 측면 전후 비교는 `BeforeLeft_AfterRight_V091.mp4`(왼쪽 이전/오른쪽 수정)다. 실제 Game 화면 녹화가 아니며 실제 프로그램 시작/입력과 사용자 품질 승인은 미검증이다. **사뿐한 앉기·일어서기 사용자 확인 대기**. 로컬 저장이며 GitHub 업로드 없음.

- **최신 합의 v0.90 (2026-09-24):** 앞으로 모든 동작은 상의–다리 관통을 세밀하게 검사·미세 보정하지 않고 일부 돌출을 허용한다. 자연스러운 펄럭임·살랑거림·후행을 우선하며 움직임이 멈추면 흔들림도 잦아들게 한다. 발/손/엉덩이 접지·무릎 방향·동작 연결·떨림 등 동작 자체의 품질 확인은 유지한다.
- 기존 걷기·달리기와 해당 실행용 보정은 그대로 둔다. 앉기·일어서기 후속 작업과 이후 모든 동작에 새 기준을 적용한다. 아래 고밀도 코트 검사 결과와 세밀한 보완 요구는 역사 기록이며 향후 필수 완료 조건으로 승계하지 않는다.
- 이번에는 MasterPlan·Progress·앉기 검토 문서만 수정했다. 코드·기존 검사 도구·자산·실행 파일은 그대로이며 옷 흔들림 구현이나 기존 보정 제거는 수행하지 않았다. 문서 기준 일치만 확인했고 Unity 검사/빌드/실제 입력은 다시 실행하지 않았다. 로컬 저장이며 GitHub 업로드는 하지 않았다. **앉기·일어서기 사용자 확인 대기**는 유지한다.

- **최신 v0.89 (2026-09-24):** 앉기 48프레임/1.6초·일어서기 54프레임/1.8초 비반복 클립과 정지된 앉은 기준 자세를 추가했다. 양손 짚기·발 정리·체중 이동·귀/목도리/배낭 후행을 포함한다. 앉은 호흡/시선 반복은 아직 없다. 이동 중 요청은 기존 정지 완료 후 시작하고 전환 중 반복/반대 요청을 무시하며 끝에서 자세를 유지한다. 검토 UI는 v0.89, 8포즈·배속·기존 카메라 조작을 제공한다.
- 앉기 전용 실행용 코트 복제본으로 무릎 앞쪽·허벅지 위·밑단을 보정한다. 낮은 면 수의 코트 정점 사이로 무릎이 나오던 부분은 정점뿐 아니라 면 전체를 검사하고 이웃 면으로 보정을 분산해 수정했다. 배낭 앞쪽 끈은 별도 실행용 복제본에서 코트 표면으로 이동하며 원본은 보존한다. 깊게 굽힐 때 끈 끝과 코트 접합이 완전히 매끈한지는 사용자 확인 대상으로 남긴다.
- 최종 자동 검사: 두 전환 전체 240Hz, 손/발 고정점 이동 최대 0.997mm(기준 3.5mm), 지지 접촉면 간격 4.225~4.831mm(5mm 이하), 최종 착석 바지 바닥 4.657mm. 전체 신발 최저 +0.422mm, 손 +4.225mm, 바지 +4.461mm, 관절 길이 차이 약 0.001mm, 좌우 순서 기준 무릎 간격 최소 296.096mm. 코트/배낭 지면 침범 없음. 근거 `AdultRabbitSitVerification.txt`와 `AdultRabbitSitMotion.csv`.
- 30/60/120fps×0.5×/1×, 일시정지, 끝 자세 유지, 반복/반대 요청, 이동 후 착석, 명시적 비활성화 함수 호출·재생성·원본 메시 복구 통과. 앉기 끝→일어서기 시작 정점 차이는 보고 정밀도 0. 완료 판정은 Unity 클립 길이의 미세 소수점 차이에 10μs 허용 오차를 둔다. 실제 Play Mode 재활성화·버튼/마우스 입력 확인과는 구분한다.
- 기존 걷기 코트 4,220표본/접지·전환 회귀, 달리기 48정지/15,472시점·부드러움 검사 통과. 마지막 앉기 기울기 수정 이후에는 앉기 검사와 빌드를 다시 실행했다. 작업 전 로컬 `Logs/BeforeV089/AdultRabbitMotion.blend`와 비교해 기존 5개 클립의 F-curve(키/핸들/보간), 메시 정점/면, 바인드 뼈 해시가 모두 동일함을 `Tools/verify_sitting_preservation.py`로 확인했다. 기존 농가·원본 코트·몸체는 변경하지 않았다.
- Windows 빌드: `Logs/AdultRabbitMotionPlayer/TinyDaysAdultRabbitMotion.exe`, 데이터 파일 갱신 2026-09-24 19:20:30. `ADULT_SIT_OK`/`ADULT_SIT_SEQUENCE_OK`/`ADULT_RABBIT_MOTION_PLAYER_OK` 및 기존 회귀 성공 마커 확인. 소스·Blender·FBX·생성 장면·검사/생성 도구·참고 이미지·문서를 로컬 보존했다. 실제 프로그램 시작/입력 검증과 GitHub 업로드는 이번에 하지 않았다.
- 정면·측면·후측면 8포즈와 연속 표본을 확인했다. `Captures/AdultRabbitSit/SitStandV089.mp4`는 1×·30fps·4.2초의 자동 렌더이며 실제 화면 녹화가 아니다. 깊게 굽히는 구간의 코트/끈 접합, 전환 리듬의 자연스러움과 모든 방향·시점의 무관통은 수치 통과만으로 보장하지 않는다.
- 다음: 검토 프로그램에서 앉기→기본 자세→일어서기, 0.5×/1×·이동 후 앉기를 사용자 확인한다. **앉기·일어서기 사용자 확인 대기**이며 확인 후 앉은 상태의 호흡·작은 시선/무게 이동을 계획한다. 달리기의 별도 최종 품질 승인은 추정하지 않았다. 눕기·농가 적용·GitHub 업로드는 하지 않았다. 아래 기록은 이전 단계 이력이다.
...
현재 캐릭터 작업의 최신 자동 결과는 [AdultCommonIdleVerification.txt](AdultCommonIdleVerification.txt), 품질/한계는 [AdultCommonIdleReview.md](AdultCommonIdleReview.md)를 따른다. 아래 농가 표는 별도 미완료 관찰 이력이며 캐릭터의 실제 입력 검증 근거가 아니다.
...
3. `TinyDays/Docs/MasterPlan.md` → 이 문서 → 현재 캐릭터 작업은 `TinyDays/Docs/AdultCommonIdleReview.md` 순으로 읽는다. 농가 재개 시에는 `Stage273Observation.md`를 추가 확인한다.
4. 현재 캐릭터는 `Assets/Scenes/AdultRabbitMotionStudy.unity`, 농가 작업은 `Assets/Scenes/FarmStudy.unity`에서 재개한다.
