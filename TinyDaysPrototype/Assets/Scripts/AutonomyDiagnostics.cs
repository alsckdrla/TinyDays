...
namespace TinyDays
{
    public sealed class AutonomyDiagnostics : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            ...
            var expansion=GetComponent<VillageExpansion>(); if(expansion) GUILayout.Label("저장시설 | "+expansion.StatusText);
            var performance=GetComponent<PerformanceMonitor>(); if(performance) GUILayout.Label(performance.Summary);
            GUILayout.EndArea();
        }
    }
}
